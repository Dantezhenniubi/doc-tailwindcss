...
<template>
  ...
  >
    ...
    >
      ...
      <text v-if="ready" xml:space="preserve" :class="`${textColor} ${className}`">
        <textPath
          ref="textPathRef"
          :href="`#${pathId}`"
...
interface CurvedLoopProps {
  marqueeText?: string; //  滚动文字内容
  textColor?: string; //  滚动文字颜色
  speed?: number; //  滚动速度
  className?: string; //  自定义样式类
  curveAmount?: number; //  曲线幅度
  direction?: 'left' | 'right'; //  默认滚动方向
